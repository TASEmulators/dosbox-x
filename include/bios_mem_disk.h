...
#ifndef DOSBOX_BIOS_MEM_DISK_H
#define DOSBOX_BIOS_MEM_DISK_H
...
class imageMemDisk {
	...
	protected:
		imageMemDisk(IMAGE_TYPE class_id);
	...
	public:
		...
		int Release() {
			int ret = --refcount;
			if (ret < 0) {
				fprintf(stderr,"WARNING: imageMemDisk Release() changed refcount to %d\n",ret);
				abort();
			}
...
class imageMemDiskEmptyDrive : public imageMemDisk {
public:
	...
	imageMemDiskEmptyDrive();
	virtual ~imageMemDiskEmptyDrive();
};

class imageMemDiskD88 : public imageMemDisk {
public:
	uint8_t Read_Sector(uint32_t head,uint32_t cylinder,uint32_t sector,void * data,unsigned int req_sector_size=0) override;
	uint8_t Write_Sector(uint32_t head,uint32_t cylinder,uint32_t sector,const void * data,unsigned int req_sector_size=0) override;
	uint8_t Read_AbsoluteSector(uint32_t sectnum, void * data) override;
...
class imageMemDiskNFD : public imageMemDisk {
public:
	uint8_t Read_Sector(uint32_t head,uint32_t cylinder,uint32_t sector,void * data,unsigned int req_sector_size=0) override;
	uint8_t Write_Sector(uint32_t head,uint32_t cylinder,uint32_t sector,const void * data,unsigned int req_sector_size=0) override;
	uint8_t Read_AbsoluteSector(uint32_t sectnum, void * data) override;
...
class imageMemDiskVFD : public imageMemDisk {
public:
	uint8_t Read_Sector(uint32_t head,uint32_t cylinder,uint32_t sector,void * data,unsigned int req_sector_size=0) override;
	uint8_t Write_Sector(uint32_t head,uint32_t cylinder,uint32_t sector,const void * data,unsigned int req_sector_size=0) override;
	uint8_t Read_AbsoluteSector(uint32_t sectnum, void * data) override;
...
class imageMemDiskMemory : public imageMemDisk {
public:
	uint8_t Read_AbsoluteSector(uint32_t sectnum, void * data) override;
	uint8_t Write_AbsoluteSector(uint32_t sectnum, const void * data) override;
	...
	imageMemDiskMemory(uint32_t imgSizeK);
	// Create a hard drive image of a specified geometry
	imageMemDiskMemory(uint16_t cylinders, uint16_t heads, uint16_t sectors, uint16_t sector_size);
	// Create a floppy image of a specified geometry
    imageMemDiskMemory(const diskGeo& floppyGeometry);
	// Create a copy-on-write memory image of an existing image
	imageMemDiskMemory(imageMemDisk* underlyingImage);
	virtual ~imageMemDiskMemory();

private:
	void init(diskGeo diskParams, bool isHardDrive, imageMemDisk* underlyingImage);
    ...
    imageMemDisk* underlyingImage = NULL;

	diskGeo floppyInfo;
};

class imageMemDiskVHD : public imageMemDisk {
public:
	enum ErrorCodes : int
	{
		OPEN_SUCCESS = 0,
	...
	uint8_t Read_AbsoluteSector(uint32_t sectnum, void * data) override;
	uint8_t Write_AbsoluteSector(uint32_t sectnum, const void * data) override;
	static ErrorCodes Open(const char* fileName, const bool readOnly, imageMemDisk** disk);
...
private:
	...
	imageMemDiskVHD() : imageMemDisk(ID_VHD) { }
    static ErrorCodes TryOpenParent(const char* childFileName, const ParentLocatorEntry& entry, const uint8_t* data, const uint32_t dataLength, imageMemDisk** disk, const uint8_t* uniqueId);
	static ErrorCodes Open(const char* fileName, const bool readOnly, imageMemDisk** disk, const uint8_t* matchUniqueId);
    ...
    imageMemDisk* parentDisk = NULL;
    imageMemDisk* fixedDisk = NULL;
...
class imageMemDiskElToritoFloppy : public imageMemDisk {
public:
    ...
    virtual ~imageMemDiskElToritoFloppy() {
    }
...
class imageMemDiskINT13Drive : public imageMemDisk {
public:
	uint8_t Read_Sector(uint32_t head,uint32_t cylinder,uint32_t sector,void * data,unsigned int req_sector_size=0) override;
	uint8_t Write_Sector(uint32_t head,uint32_t cylinder,uint32_t sector,const void * data,unsigned int req_sector_size=0) override;
	uint8_t Read_AbsoluteSector(uint32_t sectnum, void * data) override;
	...
	imageMemDiskINT13Drive(imageMemDisk *sdisk);
	virtual ~imageMemDiskINT13Drive();

	uint8_t bios_disk = 0;
	bool enable_int13 = false;
	imageMemDisk* subdisk = NULL;
	bool busy = false;
};
